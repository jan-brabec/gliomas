function plot_appendix_all_MKI_positive()

load('../dat.mat');
limit = 2.05;
ind = find([dat.mki_vs_wm] > limit);
mki = [dat(ind).bof]';
...
for c_exp = 1:numel(mki)
    ...
    for c = 1:size(sss,1)
        if strcmp(sss{c,2},num2str(mki(c_exp))) && sss{c,1} == 9999
            ...
            contrast_name_list = {'dtd_covariance_MKi.nii.gz'};
            
            I_mask = mdm_nii_read(fullfile(data_dir,'mask.nii.gz'));
            I_mask = I_mask(:,:,mki_slice);
            
            I_roi = mdm_nii_read(fullfile('../../data/roi_Enhancements',strcat(subject_name(end-2:end),'_', ss{2},'_MKI_enh.nii.gz')));
            I_roi = I_roi(:,:,mki_slice);
            
            I_roi_wm = mdm_nii_read(fullfile('../../data/roi_Enhancements',strcat(subject_name(end-2:end),'_', ss{2},'_WM_contra.nii.gz')));
            I_roi_wm = I_roi_wm(:,:,mki_slice);                        
...
print(sprintf('All_MKI.png'),'-dpng','-r500')
