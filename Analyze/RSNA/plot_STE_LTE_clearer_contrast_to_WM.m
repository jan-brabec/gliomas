...
for c_exp = 1:numel(c_subject)
    ...
    ss{c_exp,3} = sss{4};
    ss{c_exp,4} = [(sss{5}) (sss{6})];
    ss{c_exp,5} = [(sss{7}) (sss{8})];
    ss{c_exp,6} = [(sss{9}) (sss{10})];
    ss{c_exp,7} = [(sss{11}) (sss{12})];
...
% Axis setup
n_c = 6;
fh = m_upper + enh  + n_exam * 2 * ph  + sum(phm)  + m_lower; %3
...
print(sprintf('contrast_to_WM.png'),'-dpng','-r300')
